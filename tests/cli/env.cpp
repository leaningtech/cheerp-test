...
// RUN: %run_if_js %compile -cheerp-make-module=es6 -I%S/../ -o %t-env.mjs %s 2>&1
// RUN: %run_if_js python3 %helpers/create_driver.py %t-env.mjs %S/env.testing.js %t-env_driver.mjs --module=es6
// RUN: %run_if_js %node %t-env_driver.mjs 2>&1 | %FileCheck %s
