...
// It compiles test4.cpp twice without corruption and expects the outputs to match.
//
// RUN: %cheerp_clang -O1 -frtti -I%S/../.. -cheerp-bounds-check -cheerp-fix-wrong-func-casts -target cheerp %S/../test4.cpp -o %t_reference.js
// RUN: %node %t_reference.js 2>&1 > %t_reference.out
// RUN: %cheerp_clang -O1 -frtti -I%S/../.. -cheerp-bounds-check -cheerp-fix-wrong-func-casts -target cheerp %S/../test4.cpp -o %t_buggy.js
