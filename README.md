...
```bash
python3 ./cheerp-test/run_lit_tests.py --help
```
...
python3 ./cheerp-test/run_lit_tests.py --suite ./cheerp-test

# Equivalent from inside cheerp-test/
...
python3 tests/run_lit_tests.py \
  --compiler /opt/cheerp2/bin/clang++ \
  --cheerp-prefix /opt/cheerp2 \
  tests

# Parallel jobs and optimization level
python3 tests/run_lit_tests.py -j8 -O2 tests

# Preexecute modes
python3 tests/run_lit_tests.py --preexecute tests
python3 tests/run_lit_tests.py --preexecute-asmjs tests

# Determinism checks (in addition to selected target modes)
python3 tests/run_lit_tests.py --determinism 5 --determinism-probability 0.2 tests
```
...
lit --param OPT_LEVEL=O2 --param CHEERP_FLAGS='-cheerp-pretty-code' tests/unit
```
...
- `TARGET` : comma-separated subset of `js,wasm,asmjs`
...
- `CHEERP_PREFIX` : toolchain prefix (e.g. `/opt/cheerp2`)

## Reports and outputs

When using `tests/run_lit_tests.py`, these files are generated in `cheerp-test/`:
